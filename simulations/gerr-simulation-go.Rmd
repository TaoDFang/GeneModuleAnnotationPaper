...
library(R.cache)
setCacheRootPath(file.path(getwd(), 'cache'))
```
...
When performing GO enrichment, usually GO-terms are propagated from child nodes to their parents. In the code below we select random subset of GO-categories and propagate them up to the root of the tree. In this way the number of GO-terms tested will be larger than the number of terms chosen.
...
selRes <- evalWithMemoization(
  Q(function(i, goi, gsMatrix)
...
```{r verificateAll, message=FALSE}
verifGerr <- evalWithMemoization(
  Q(runGerr,
...
```{r fetFdrVerif, message=FALSE}
fisherVerif <- evalWithMemoization(
  Q(fetFdr,
    goi=simGenesets,
...
```{r topGOestimate, message=FALSE}
topgo_results <- evalWithMemoization(
  map_dfr(topgo_methods, function(method)
    Q(estimate_topgo,
      gs=simGenesets,
      tgData=list(tgData),
      method=method,
...
```{r testProbSim, message=FALSE}
testSim <- evalWithMemoization(
  probSim(p_gs=0.05, p_g=0.5, p_n=1E-3) %>%
...
```{r fullEst, message=FALSE}
full_sim_res <- evalWithMemoization(
   Q(
